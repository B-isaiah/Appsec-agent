...
Begin. Test every endpoint."""
    ...
    print(f"\n{C.BOLD}[AGENT]{C.RESET} Claude starting — {len(eps)} endpoints\n")
    ...
    while not done and iters < 150:
        ...
        for blk in resp.content:
            if blk.type == "text" and blk.text.strip():
                print(f"\n{C.DIM}[Agent] {blk.text[:160]}{C.RESET}")
                continue
            ...
            if blk.name == "probe":
                ...
                result   = probe(inp["method"],inp["url"],inp.get("body"),
                                 identity, base_url, inp.get("extra_headers"))
                results.append({"type":"tool_result","tool_use_id":blk.id,
                                 "content":json.dumps(result)})

            elif blk.name == "report_finding":
                report_finding(inp["title"],inp["severity"],inp["owasp_category"],
                               inp["description"],inp["op_ids"],inp["remediation"])
                results.append({"type":"tool_result","tool_use_id":blk.id,"content":"Logged."})

            elif blk.name == "testing_complete":
                print(f"\n{C.BOLD}[DONE]{C.RESET} {inp['summary']}")
                print(f"  Tested: {len(inp.get('tested_urls',[]))} | Findings: {len(findings)}")
                results.append({"type":"tool_result","tool_use_id":blk.id,"content":"Done."})
                done = True
